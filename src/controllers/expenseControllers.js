...
exports.addExpense = async (req,res) => {
    try {
        ...
        if(!expenseName || !expenseType){
            return res.status(400).json({success: false, message: "Expense name and type are required"});
        }
        if(!amount || isNaN(amount) || amount<=0){
            return res.status(400).json({success: false, message: "Amount must be a valid positive number"});
        }
        const newExpense = await Expense.create({userId,expenseName,amount,expenseType});
        console.log(newExpense);
        res.status(201).json({success: true, message: "New Expense created", expense: newExpense});
    } catch(error) {
...
exports.editExpense = async (req,res) => {
    try {
        ...
        res.status(500).json({success: false, message: "Failed to update the Expense", error: error.message});
    }
};
...
exports.deleteExpense = async (req,res) => {
    try {
        ...
        res.status(200).json({success: true, message: "Deleted the Expense Successfully"});
    } catch(error) {
        res.status(500).json({success: false, message: "Error in deleting the expense", error: error.message});
    }
};